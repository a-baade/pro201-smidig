...
function UserCard({ organization: { jobTitle,  img: { image }, firstName, lastName, companyName} }) {
    return (
        <div>
            <h2>{companyName}</h2>
            <p>{firstName } {lastName}</p>
            {image && (
                <img src={`data:image/png;base64${image}`} style={{ width: "200px" }} alt={"Profile Pic"} />
            )}
            <p>{jobTitle}</p>
        </div>
    );
...
export function ListUsers() {
    ...
    useEffect(() => {
        if (search === "") {
            ...
            fetchJSON(`/api/register/search/?companyName=${search}`).then((jsonData) => {
                setData(jsonData.register);
            });
        }
    }, [search]);
